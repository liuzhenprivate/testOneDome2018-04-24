...
<body onload="CrbtOrders()">
	...
	<input type="hidden" id="RECHARGE_NAME" name="RECHARGE_NAME" value="${pd.RECHARGE_NAME }"/>
	<div class="generalRechargeSetDe">
		<h1>充值设置</h1>
		<div class="generalRechargeSetDeInp clearfix">
			<span>充值金额</span>
			<div class="clearfix">
				<input name="MONEY" id="MONEY" value="${pd.MONEY/100 }" onKeyUp="BOOK_CURRENCYVAL();" type="text" placeholder="请输入充值金额单位元"/>
			</div>
		</div>
		<div class="generalRechargeSetDeNor clearfix">
			<span>阅读币</span>
			<p><i id="BOOK_CURRENCY1">${pd.BOOK_CURRENCY }</i></p>个
			<input type="hidden" name="BOOK_CURRENCY" id="BOOK_CURRENCY" value="${pd.BOOK_CURRENCY }" />
		</div>
		<div style="margin-top: 9px;" class="generalRechargeSetDeInp clearfix">
			<span>赠送</span>
			<div class="clearfix">
				<input  name="BOOK_CURRENCY_GIFT" id="BOOK_CURRENCY_GIFT" value="${pd.BOOK_CURRENCY_GIFT}" type="text" placeholder="请输入赠送阅读币数量"/>
			</div>
		</div>
		<div class="generalRechargeSetDeSel clearfix">
			<span>选择可用范围</span>
			<div class="generalRechargeSetDeSelDiv clearfix">
				<p><c:if test="${pd.USE_SCOPE  == 0 }">所有读者</c:if><c:if test="${pd.USE_SCOPE  == 1 }">VIP读者</c:if></p>
				<input type="hidden" name="USE_SCOPE" id="USE_SCOPE" value="${pd.USE_SCOPE}">
		...
		<div class="generalRechargeSetDeInp clearfix">
			...
			<c:choose>
				<c:when test="${not empty var.EXP_DATE}">
					<input id="EXP_DATE" name="EXP_DATE"  value="${pd.EXP_DATE}" style="width:455px; margin-right:10px;"> 
				</c:when>
				<c:otherwise>
		...
		<div class="generalRechargeSetDeInp clearfix datevalue">
			...
			<div class="clearfix">
				...
				<input  name="SORT_NUM" id="SORT_NUM" value="${pd.SORT_NUM}" type="text" placeholder="请输入排列序号"/>
			</div>
		</div>
		<div class="generalRechargeSetDeTex clearfix">
			<span>备注</span>
			<div class="clearfix">
				<textarea  name="MEMO" id="MEMO" placeholder="请输入备注">${pd.MEMO}</textarea>
			</div>
		</div>
	...
	<script>
		function rechargeset(){
			...
			$("#rechargeForm").attr("action","<%=basePath%>rechargeset/edit.do");
			$("#rechargeForm").submit();
		}
	</script>
	<script>
