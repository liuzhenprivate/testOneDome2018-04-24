...
<script type="text/javascript">
	...
	function save(){
		...
		if($("#ARTICLE_CHAPTER_ID").val()==""){
			$("#ARTICLE_CHAPTER_ID").tips({
				side:3,
	            msg:'请输入用户阅读最新章节ID',
	            bg:'#AE81FF',
	            time:2
	        });
			$("#ARTICLE_CHAPTER_ID").focus();
			return false;
		}
		if($("#CREATE_TIME").val()==""){
			$("#CREATE_TIME").tips({
...
<body>
	<form action="bookshelf/${msg }.do" name="Form" id="Form" method="post">
		<input type="hidden" name="BOOKSHELF_ID" id="BOOKSHELF_ID" value="${pd.BOOKSHELF_ID}"/>
		<div id="zhongxin">
		<table id="table_report" class="table table-striped table-bordered table-hover">
			...
			<tr>
				...
				<td><input type="text" name="USERID" id="USERID" value="${pd.USERID}" maxlength="32" placeholder="这里输入读者ID" title="读者ID"/></td>
			</tr>
			<tr>
				<td style="width:70px;text-align: right;padding-top: 13px;">渠道ID:</td>
				<td><input type="text" name="USER_ID" id="USER_ID" value="${pd.USER_ID}" maxlength="32" placeholder="这里输入渠道ID" title="渠道ID"/></td>
			</tr>
			<tr>
				<td style="width:70px;text-align: right;padding-top: 13px;">用户阅读最新章节ID:</td>
				<td><input type="text" name="ARTICLE_CHAPTER_ID" id="ARTICLE_CHAPTER_ID" value="${pd.ARTICLE_CHAPTER_ID}" maxlength="32" placeholder="这里输入用户阅读最新章节ID" title="用户阅读最新章节ID"/></td>
			</tr>
			<tr>
